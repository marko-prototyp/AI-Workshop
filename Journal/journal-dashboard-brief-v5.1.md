...
Known confidence gaps. Each one is owned by a punchlist item below — fix the punchlist item, the gap closes.
...
**Acceptance.** Run the **Image smoke test** in the Image flow section. All 12 steps must pass without manual fixup. The figure renders correctly in `dist/journal/NN.html` with the right `src`, `alt`, and `figcaption`.

**Likely follow-up.** If step 10 fails (image not copied into `dist/assets/weeks/NN/`), add the copy step to `build.js`:

```txt
Journal/week-images/NN/*  →  dist/assets/weeks/NN/
```
...
**Acceptance.** Create a small manual or automated regression check that passes every bullet under **Frontmatter regression checks** in the Public Entry file format contract, plus one additional case: a missing Public Entry initializes from `weeks.json` correctly when first opened.
...
Active reference. Supersedes v5. Earlier briefs (v3, v4.2, v5, and the React decision brief) are kept for history.
